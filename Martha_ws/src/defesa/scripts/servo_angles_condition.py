...
pub_distance = rospy.Publisher("distance_ball", Float32, queue_size = 1)
#time.sleep(5)
...
def angles_callback(msg):#toda vez wue eu receber a posicao da bola eu vou executar o callback
    ...
    if stop_publishing == False:
        ...
        rospy.loginfo("vou publicar nada não")
    ...
    valor_recebido = np.float32(ball_distance.data)
    ...
    if valor_recebido > limite32:
        rospy.loginfo("a distancia esta maior que -13")
        stop_publishing == True
    else:
        rospy.loginfo("a distancia esta menor que -13")    
        stop_publishing == False
    #time.sleep(0.8)

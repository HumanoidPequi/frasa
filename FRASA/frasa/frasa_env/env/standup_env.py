...
class StandupEnv(gymnasium.Env):
    ...
    def apply_control(self, q: np.ndarray, reset: bool = False) -> None:
        ...
        if reset:
            for k, dof in enumerate(self.dofs):
                self.sim.set_control(dof, q[k], True)
    ...
    def get_observation(self) -> np.ndarray:
        ...
        # Current control
        ctrl = [self.sim.get_control(dof) for dof in self.dofs]
        ctrl = np.array(ctrl)
    ...
    def reset(
        ...
        self.sim.set_control("l_sho_roll_link", -np.deg2rad(5))
